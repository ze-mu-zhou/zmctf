...
# 跨平台tamper命令：rev/cat是Unix工具，Windows上没有，用python单行命令等价替代
TAMPER_REVERSE = (
    "rev"
    if os.name != "nt"
    else sys.executable
    + ' -c "import sys;sys.stdout.write(sys.stdin.read()[::-1])"'
)
TAMPER_IDENT = (
    "cat"
    if os.name != "nt"
    else sys.executable + ' -c "import sys;sys.stdout.write(sys.stdin.read())"'
)
...
class TestCLI(unittest.TestCase):
    ...
    def test_crack_request_basic(self):
        ...
        # Windows上NamedTemporaryFile句柄未关闭时无法再次打开，先关闭再写入
        temp_file.close()
        # 写入数据到临时文件
