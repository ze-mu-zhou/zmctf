...
class TestBase(unittest.TestCase):
    ...
    def test_waf(self):
        ...
        # 目标为Windows时os.popen走cmd：单引号不保护;&&&等元字符，命令会被截断，
        # 换用cmd安全的标记串（POSIX目标仍用含元字符的完整标记串）
        marker = 'cracked! @m WR171NG[]{}|;&&&" S()METHING RANDON'
        if os.name == "nt":
            marker = "cracked WR171NG SOMETHING RANDON"
        ...
        self.assertIn(
            marker, resp.text, f"{resp.text=} {payload=}"
        )
