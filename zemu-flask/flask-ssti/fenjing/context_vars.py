...
def _render_const_expr(expr: str, fallback: Any) -> Any:
    """在本地渲染常量表达式得到其值（部分表达式的值依赖平台/Jinja2版本，
    如lipsum函数repr中的地址位数），渲染失败时回退到硬编码值

    Args:
        expr (str): Jinja2表达式
        fallback (Any): 渲染失败时的回退值
    ...
    try:
        ...
        return type(fallback)(Template("{{(" + expr + ")}}").render())
...
const_exprs: ContextExpression = {
    ...
    # 该值来自lipsum函数repr（含内存地址），地址位数随平台/Jinja2版本变化，
    # 导入时本地渲染自适应，失败则回退到作者机器上的硬编码值
