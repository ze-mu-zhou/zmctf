...
class HTTPRequester:
    ...
    def __init__(
        ...
        # 环回地址始终绕过代理：requests默认继承系统/环境代理（如本机Clash），
        # 而打本机靶机（127.0.0.1等）走代理会被重置，导致全部请求失败
        no_proxy = os.environ.get("no_proxy") or os.environ.get("NO_PROXY", "")
        bypass_hosts = ["localhost", "127.0.0.1", "::1"]
        missing = [h for h in bypass_hosts if h not in no_proxy.split(",")]
        if missing:
            no_proxy = (
                f"{no_proxy},{','.join(missing)}" if no_proxy else ",".join(missing)
            )
