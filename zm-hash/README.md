...
实测（RTX 4060 Laptop，长度 10 数字全空间 1e10 候选）：内核解码改造后同窗对比已**超过**同机 hashcat 7.1.2 MD5 基准约 8%~12%（电池功耗墙下 20.7 vs 18.4 GH/s；插电工况双方绝对值随时钟上浮，相对差保持）；CPU 32 线程 AVX-512 约 0.93 GH/s。注意 `--max-candidates` 下 GPU 路径按跨步子集覆盖（数量语义不变，但不保证是字典序前缀）。

调试/调优环境变量：`ZM_DUMP_KERNEL=路径` 导出现场生成的内核源码与 ptxas 寄存器报告（`.log`）；`ZM_VEC=4|8|16`（默认 16）、`ZM_LOCAL=128|256|512`（默认 256）覆盖发射参数用于调优实验。
