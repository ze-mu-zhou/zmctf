 ...
 * <p>
 * Vineflower is the modern, actively maintained successor of FernFlower.
 * It keeps FernFlower's package names and CLI option keys for compatibility.
 *
 * @author Konloch
 * @author WaterWolf
 * @since 09/26/2011
...
public class VineflowerDecompiler extends AbstractDecompiler
{
    ...
    {
        super("Vineflower Decompiler", "vineflower");
    }
    ...
    public String decompileClassNode(ClassNode cn, byte[] bytes)
    {
        ...
        try
        {
                ...
                ProcessUtils.runDecompilerExternal(ArrayUtils.addAll(new String[]
                    {
                        ...
                        "-jar", ExternalResources.getSingleton().findLibrary("vineflower")
            ...
            //Vineflower always names the output file after the class's internal name,
            //not after the (randomized) input file name like the old FernFlower did
            int indexOfLastPackage = cn.name.lastIndexOf('/');
            String classNameNoPackages = indexOfLastPackage < 0 ? cn.name : cn.name.substring(indexOfLastPackage + 1);
            File classNameOutput = new File(tempFile.getParent(), classNameNoPackages + ".java");
            if (classNameOutput.exists())
            {
                tempOutputJavaFile = classNameOutput;
            ...
            if (tempOutputJavaFile.exists() && !Constants.DEV_FLAG_DECOMPILERS_SIMULATED_ERRORS)
                ...
                exception = VINEFLOWER + " " + ERROR + "! " + tempOutputJavaFile.getAbsolutePath() + " does not exist.";
        }
        catch (Throwable e)
        {
        ...
        return VINEFLOWER + " " + ERROR + "! " + ExceptionUI.SEND_STACKTRACE_TO + NL + NL
